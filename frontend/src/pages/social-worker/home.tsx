  ...
  return (
    <div className="page-content">
      ...
      <div className="stat-cards">
        <div className="stat-card"><p className="stat-label">Active Cases</p><p className="stat-value">{cases.length}</p></div>
        <div className="stat-card"><p className="stat-label">Confirmed Events</p><p className="stat-value">{confirmedEvents.length}</p></div>
        <div className="stat-card"><p className="stat-label">Pending Referrals</p><p className="stat-value stat-value--alert">{pendingReferrals.length}</p></div>
      </div>
        ...
        {cases.map(c => {
          ...
          return (
            <Link key={c.id} to="/sw/active-cases" className="case-row">
              ...
              <span className="risk-badge" style={{ background: risk.bg, color: risk.text }}>{risk.label}</span>
            </Link>
          );
        })}
      </div>
